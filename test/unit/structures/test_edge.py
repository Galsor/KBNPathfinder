...
def test_edge_constructor(mock_node_pair):
    ...
    assert set([node.id for node in e.nodes]) == set([node.id for node in mock_node_pair])
...
def test_get_dest_relative_score(mock_euclidian_edge):
    ...
    rel_score = mock_euclidian_edge.get_dest_relative_score(origin_node_id=org_node_id, max_cost=100)
