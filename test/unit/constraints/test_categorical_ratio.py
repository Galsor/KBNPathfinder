...
@pytest.fixture
def mock_flexible_categorical_ratio_constraint(
    random_graph, mock_constraints
) -> FlexibleCategoricalRatioConstraint:
    property_name, objectives, k = mock_constraints
    return FlexibleCategoricalRatioConstraint(
        property_name, objectives, random_graph, k
    )
...
def test_constructor(mock_flexible_categorical_ratio_constraint, mock_constraints):
    ...
    assert (
        mock_flexible_categorical_ratio_constraint.category_counts
        == expected_cat_counts
    )
    assert (
        mock_flexible_categorical_ratio_constraint.objectives_counts
        == expected_objectives_counts
    )


def test_compute_category_means(
    mock_flexible_categorical_ratio_constraint, random_graph
):
    cat_means = mock_flexible_categorical_ratio_constraint.compute_category_means(
        random_graph
    )
    print(cat_means)
...
def test_update_state(mock_flexible_categorical_ratio_constraint, random_graph):
    ...
    assert mock_flexible_categorical_ratio_constraint.category_counts == {
        "A": 0,
        "B": 1,
        "C": 0,
    }
...
@pytest.mark.parametrize(
    "mock_category_counts, expected_completion_states",
    [
        (
            {"A": 0, "B": 0, "C": 0},
            {"A": 0.0, "B": 0.0, "C": 1.0},
        ),  # C has a low objective so completed by default
...
def test_completion_state(
    ...
    mock_flexible_categorical_ratio_constraint,
):
...
@pytest.mark.parametrize(
    "nodes_added, candidate, expected_penalization_ratio",
    [
        ([0, 1, 2], 3, 0.55),
        ([0, 1, 2], 4, 0),
        ([0, 1, 2], 9, 1.0),
...
def test_penalize_score(
    nodes_added,
    candidate,
    expected_penalization_ratio,
    mock_flexible_categorical_ratio_constraint,
    random_graph,
):
    ...
    for node_id in nodes_added:
        mock_flexible_categorical_ratio_constraint.update_state(
            random_graph.nodes[node_id]
        )
    penalized_score = mock_flexible_categorical_ratio_constraint.penalize_score(
        candidate_node
    )
