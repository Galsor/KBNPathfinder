...
def find_next_best_neighbors(graph: KBNGraph, selected_nodes: List[Node], node_count_to_add: int) -> List[Node]:
    """ Recursivly find k best contiguous nodes """
    last_node = selected_nodes[-1]
    excluded_nodes_ids = [node.id for node in selected_nodes]
    next_node = get_neighboor_with_max_regional_score(graph, last_node.id, excluded_nodes_ids, node_count_to_add)
    if next_node is not None:
        selected_nodes.append(next_node)
    if node_count_to_add > 1:
        # Pursue recursion
        selected_nodes = find_next_best_neighbors(graph, selected_nodes, node_count_to_add=node_count_to_add-1)
    return selected_nodes

def get_neighboor_with_max_regional_score(graph: KBNGraph, node_id: int, excluded_node_ids_list: List[int], amount_of_neighbors_to_compare: int) -> Optional[Node]:
    ...
    for edge in neigh_edges:
        ...
        if node.id not in excluded_node_ids_list:
            regional_score = graph.get_node_regional_score(node.id, amount_of_neighbors_to_compare)
